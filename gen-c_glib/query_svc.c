...
static void 
query_svc_if_interface_init (querySvcIfInterface *iface);

G_DEFINE_TYPE_WITH_CODE (querySvcClient, query_svc_client,
                         G_TYPE_OBJECT, 
                         G_IMPLEMENT_INTERFACE (TYPE_QUERY_SVC_IF,
                                                query_svc_if_interface_init))

enum _querySvcClientProperties
{
  PROP_0,
  PROP_QUERY_SVC_CLIENT_INPUT_PROTOCOL,
  PROP_QUERY_SVC_CLIENT_OUTPUT_PROTOCOL
};

void
query_svc_client_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
  querySvcClient *client = QUERY_SVC_CLIENT (object);

  THRIFT_UNUSED_VAR (pspec);

  switch (property_id)
  {
    case PROP_QUERY_SVC_CLIENT_INPUT_PROTOCOL:
      client->input_protocol = g_value_get_object (value);
      break;
    case PROP_QUERY_SVC_CLIENT_OUTPUT_PROTOCOL:
...
void
query_svc_client_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
  querySvcClient *client = QUERY_SVC_CLIENT (object);
...
gboolean query_svc_client_recv_execute (querySvcIf * iface, I_QueryResult ** _return, GError ** error)
{
  ...
  ThriftProtocol * protocol = QUERY_SVC_CLIENT (iface)->input_protocol;
...
gboolean query_svc_client_execute (querySvcIf * iface, I_QueryResult ** _return, const I_Query * query, GError ** error)
{
  if (!query_svc_client_send_execute (iface, query, error))
    return FALSE;
  if (!query_svc_client_recv_execute (iface, _return, error))
    return FALSE;
  return TRUE;
}
...
query_svc_if_interface_init (querySvcIfInterface *iface)
{
  iface->execute = query_svc_client_execute;
}

static void
query_svc_client_init (querySvcClient *client)
{
...
static void
query_svc_client_class_init (querySvcClientClass *cls)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (cls);
  GParamSpec *param_spec;

  gobject_class->set_property = query_svc_client_set_property;
  gobject_class->get_property = query_svc_client_get_property;
  ...
  g_object_class_install_property (gobject_class,
                                   PROP_QUERY_SVC_CLIENT_INPUT_PROTOCOL, param_spec);
  ...
  param_spec = g_param_spec_object ("output_protocol",
                                    ...
                                    G_PARAM_READWRITE);
  g_object_class_install_property (gobject_class,
                                   PROP_QUERY_SVC_CLIENT_OUTPUT_PROTOCOL, param_spec);
}

static void
query_svc_handler_query_svc_if_interface_init (querySvcIfInterface *iface);

G_DEFINE_TYPE_WITH_CODE (querySvcHandler, 
                         query_svc_handler,
                         G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (TYPE_QUERY_SVC_IF,
...
gboolean query_svc_handler_execute (querySvcIf * iface, I_QueryResult ** _return, const I_Query * query, GError ** error)
{
  ...
  return QUERY_SVC_HANDLER_GET_CLASS (iface)->execute (iface, _return, query, error);
}

static void
query_svc_handler_query_svc_if_interface_init (querySvcIfInterface *iface)
{
  iface->execute = query_svc_handler_execute;
}

static void
query_svc_handler_init (querySvcHandler *self)
{
  THRIFT_UNUSED_VAR (self);
}

static void
query_svc_handler_class_init (querySvcHandlerClass *cls)
{
  cls->execute = NULL;
}

enum _querySvcProcessorProperties
{
  PROP_QUERY_SVC_PROCESSOR_0,
  PROP_QUERY_SVC_PROCESSOR_HANDLER
};

G_DEFINE_TYPE (querySvcProcessor,
               query_svc_processor,
               THRIFT_TYPE_DISPATCH_PROCESSOR)

typedef gboolean (* querySvcProcessorProcessFunction) (querySvcProcessor *, 
                                                       gint32,
                                                       ThriftProtocol *,
                                                       ThriftProtocol *,
                                                       GError **);
...
{
  gchar *name;
  querySvcProcessorProcessFunction function;
} query_svc_processor_process_function_def;

static gboolean
query_svc_processor_process_execute (querySvcProcessor *,
                                     gint32,
                                     ThriftProtocol *,
                                     ThriftProtocol *,
                                     GError **);

static query_svc_processor_process_function_def
query_svc_processor_process_function_defs[1] = {
  {
    "execute",
    query_svc_processor_process_execute
  }
};

static gboolean
query_svc_processor_process_execute (querySvcProcessor *self,
                                     gint32 sequence_id,
                                     ...
                                     GError **error)
{
  ...
  ThriftApplicationException *xception;
  querySvcExecuteArgs * args =
    g_object_new (TYPE_QUERY_SVC_EXECUTE_ARGS, NULL);
      ...
      (thrift_transport_read_end (transport, error) != FALSE))
  {
    ...
    result_struct = g_object_new (TYPE_QUERY_SVC_EXECUTE_RESULT, NULL);
    g_object_get (result_struct, "success", &return_value, NULL);

    if (query_svc_handler_execute (QUERY_SVC_IF (self->handler),
                                   &return_value,
                                   query,
                                   error) == TRUE)
    {
    ...
    {
      if (*error == NULL)
        g_warning ("querySvc.execute implementation returned FALSE "
...
static gboolean
query_svc_processor_dispatch_call (ThriftDispatchProcessor *dispatch_processor,
                                   ThriftProtocol *input_protocol,
                                   ThriftProtocol *output_protocol,
                                   gchar *method_name,
                                   gint32 sequence_id,
                                   GError **error)
{
  query_svc_processor_process_function_def *process_function_def;
  gboolean dispatch_result = FALSE;

  querySvcProcessor *self = QUERY_SVC_PROCESSOR (dispatch_processor);
  ThriftDispatchProcessorClass *parent_class =
    g_type_class_peek_parent (QUERY_SVC_PROCESSOR_GET_CLASS (self));
...
static void
query_svc_processor_set_property (GObject *object,
                                  guint property_id,
                                  const GValue *value,
                                  GParamSpec *pspec)
{
  querySvcProcessor *self = QUERY_SVC_PROCESSOR (object);

  switch (property_id)
  {
    case PROP_QUERY_SVC_PROCESSOR_HANDLER:
...
static void
query_svc_processor_get_property (GObject *object,
                                  guint property_id,
                                  GValue *value,
                                  GParamSpec *pspec)
{
...
static void
query_svc_processor_dispose (GObject *gobject)
{
  querySvcProcessor *self = QUERY_SVC_PROCESSOR (gobject);
  ...
  G_OBJECT_CLASS (query_svc_processor_parent_class)->dispose (gobject);
}

static void
query_svc_processor_finalize (GObject *gobject)
{
  querySvcProcessor *self = QUERY_SVC_PROCESSOR (gobject);

  thrift_safe_hash_table_destroy (self->process_map);

  G_OBJECT_CLASS (query_svc_processor_parent_class)->finalize (gobject);
}

static void
query_svc_processor_init (querySvcProcessor *self)
{
  guint index;
  ...
  for (index = 0; index < 1; index += 1)
    g_hash_table_insert (self->process_map,
                         query_svc_processor_process_function_defs[index].name,
                         &query_svc_processor_process_function_defs[index]);
}

static void
query_svc_processor_class_init (querySvcProcessorClass *cls)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (cls);
  ...
  gobject_class->dispose = query_svc_processor_dispose;
  gobject_class->finalize = query_svc_processor_finalize;
  gobject_class->set_property = query_svc_processor_set_property;
  gobject_class->get_property = query_svc_processor_get_property;

  dispatch_processor_class->dispatch_call = query_svc_processor_dispatch_call;
  cls->dispatch_call = query_svc_processor_dispatch_call;
